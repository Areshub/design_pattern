...
public class SocketMultiplexingSingleThreadv3 {
                ...
                while (selector.select(500) > 0) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        if (key.isAcceptable()) {
                            ...
                            key.cancel();
                            readHandler(key);
                            // 只处理了 read 并祖册，关心这个key 的write事件

                            // 假如起线程 非阻塞 但是存在时差 这个Key还没处理完 这个Key的read 事件会被重复触发
    ...
    public static void main(String[] args) {
        new SocketMultiplexingSingleThreadv3().start();
    }
}
